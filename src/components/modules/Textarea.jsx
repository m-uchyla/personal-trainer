...
const Textarea = ({forName, labelName, placeholderName, name, isRequired, formId}) => {
  ...
  return (
    <div className="flex flex-col my-3">
        <label 
            htmlFor={forName}   
            className={`block text-[18px] font-medium ${isFocused ? "text-primary" : "text-secondary"}`}>
        ...
        <textarea 
            ...
            rows="6" 
            className={`p-2 placeholder:text-sm rounded-md text-secondary bg-transparent border-[2px] ${(!requiredError) ? "border-gray focus:border-primary" : "border-red-600 focus:border-red-400"} valid:bg-black-gradient focus:outline-none block w-full p-2.5`}
        >
        </textarea>
